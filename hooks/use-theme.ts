      ...
      try {
        ...
        if (response.ok) {
          ...
          if (config.primaryFont) {
            console.log('Aplicando fonte via hook:', config.primaryFont)
            body.style.fontFamily = `"${config.primaryFont}", sans-serif`
          }
          
          // Aplicar tamanho da fonte
          if (config.fontSize) {
            console.log('Aplicando tamanho da fonte via hook:', config.fontSize)
            
            // Definir variável CSS para uso em todo o site
            root.style.setProperty("--font-size", `${config.fontSize}px`)
            
            // Aplicar ao html para que as unidades rem funcionem
            html.style.fontSize = `${config.fontSize}px`
          }
          
          if (config.fontWeight) body.style.fontWeight = config.fontWeight
          
          // Aplicar raio das bordas
          if (config.borderRadius !== undefined) {
            console.log('Aplicando borderRadius via hook:', config.borderRadius)
            ...
            elementsWithBorders.forEach(element => {
              ;(element as HTMLElement).style.borderRadius = `${config.borderRadius}px`
            })
          }
          
          // Aplicar intensidade de sombras
          if (config.shadowIntensity !== undefined) {
            console.log('Aplicando shadowIntensity via hook:', config.shadowIntensity)
            ...
            body.classList.add(`shadow-intensity-${config.shadowIntensity}`)
          }
        }
      } catch (error) {
